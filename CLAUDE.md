...
**Per-project vs. per-machine (a common "works in one project, not another" pitfall):** two independent steps must both be satisfied in each project. (1) The MCP server registration — `claude mcp add` defaults to **`local`** scope (visible only in the dir where it ran); use `claude mcp add -s user zipmem-mcp -- npx zipmem-mcp` to make it available in every project (or `-s project` to commit a shared `.mcp.json`). (2) `zipmem init` must run **per project**, because it creates that project's `.zipmem/state.json` *and* injects the directive into that project's `CLAUDE.md` — without the directive the agent never fires the tools even though the server is up. Also note project-dir resolution falls back to the nearest `.zipmem/` **ancestor**, so a project with no `.zipmem/` of its own can silently read/write a parent project's memory.
